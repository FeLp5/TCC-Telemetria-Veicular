...
//        SPI_DATA_OUT = 1;
        ...
        __delay_ms(10);
         ...
         do
         {
             ...
             {
                posicao_cursor_lcd(2,0);
                escreve_frase_ram_lcd("Erro B512       ");
                posicao_cursor_lcd(2,0);
                escreve_inteiro_lcd(buff);
...
////    
    if(offset == 0)
    {
        ...
        CHIP_SELECT = 1;
    //    
//        __delay_ms(1); // por conta própria
        proceed();
        do
        {   
            buff = response();
            if(buff != 0x00)
            {
                ...
                escreve_inteiro_lcd(buff);
//                __delay_ms(100); // para atrasar um pouco e ver o comportamento
            }
            else
            {
//                posicao_cursor_lcd(1,0);
...
//        __delay_ms(10);
        ...
        do
        {
            ...
            if(buff != 0xFE)
            {
                ...
                posicao_cursor_lcd(2,13);
                escreve_inteiro_lcd(buff);
//                __delay_ms(1000); // para atrasar um pouco e ver o comportamento
...
//            posicao_cursor_lcd(1,0);
//            escreve_frase_ram_lcd("length         ");
            
//            posicao_cursor_lcd(1,14);
//            escreve_inteiro_lcd(length);
//            __delay_ms(1000); // para atrasar um pouco e ver o comportamento
...
//        __delay_ms(200); // coloquei este delay por causa do git
//        SPI_DATA_OUT = 1;
        CHIP_SELECT = 0;
        length = 0;    
        dummy_clocks(8);    
        ...
        do{   //aguardando a resposta 0xFF
            buff = response();
            posicao_cursor_lcd(1,0);
            escreve_frase_ram_lcd("buffer 0xFF       ");
            posicao_cursor_lcd(1,13);
            escreve_inteiro_lcd(buff);
//            __delay_ms(1000); // para atrasar um pouco e ver o comportamento
    ...
    //    
        __delay_ms(1000); // para atrasar um pouco e ver o comportamento
        length = 0;
        while(p_buff[length]!='\0') // alterei para arr de p_buff
        {
//                LIMPA_DISPLAY();
                posicao_cursor_lcd(1,0);
...
//                __delay_ms(1000); // para atrasar um pouco e ver o comportamento
        ...
        *p_buff = length; // pointer to the pointer got in
                          // não alterei *p_buff para *rd

         return RES_OK;
    }
    else
    {
        //falta código aqui caso offset != 0
        return RES_OK; 
    }
        __delay_ms(2000);
