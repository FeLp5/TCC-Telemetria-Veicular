...
BOOL next_dir_entry (	/* TRUE: successful, FALSE: could not move next */
	DIR_ *dj				/* Pointer to directory object */
)
{
	CLUST clust;
	WORD idx;
...
FRESULT trace_path (	/* FR_OK(0): successful, !=0: error code */
	DIR_ *dj,			/* Pointer to directory object to return last directory */
...
FRESULT reserve_direntry (	/* FR_OK: successful, FR_DENIED: no free entry, FR_RW_ERROR: a disk error occured */
	DIR_ *dj,				/* Target directory to create new entry */
...
)
{
	...
	if (disk_read(0, fs->win, sect, 1) != RES_OK)	/* Load boot record */	
    {
        ...
        escreve_frase_ram_lcd("Load Boot  ");
        posicao_cursor_lcd(1,12);
        int temp = disk_read(0, fs->win, sect, 1);
        escreve_inteiro_lcd(temp);
        __delay_ms(2000);
	if (LD_WORD(&fs->win[BS_55AA]) != 0xAA55)		/* Check record signature */
    {
        ...
        escreve_frase_ram_lcd("Rec. sig.         ");
        posicao_cursor_lcd(2,10);
        unsigned long int temp2 = LD_WORD(&fs->win[BS_55AA]);
        escreve_inteiro_lcd(temp2);
        __delay_ms(3000);
	if (!memcmp(&fs->win[BS_FilSysType], "FAT", 3))	/* Check FAT signature */
		return 0;
        
        posicao_cursor_lcd(1,0);
        escreve_frase_ram_lcd("FAT >           ");
        posicao_cursor_lcd(1,8);
        unsigned long int temp3 = !memcmp(&fs->win[BS_FilSysType], "FAT", 3);
        escreve_inteiro_lcd(temp3);
        __delay_ms(3000);
//#if _FAT32
	if (!memcmp(&fs->win[BS_FilSysType32], "FAT32", 5) && !(fs->win[BPB_ExtFlags] & 0x80))
        ...
        posicao_cursor_lcd(1,0);
        escreve_frase_ram_lcd("FAT32 >           ");
        posicao_cursor_lcd(1,8);
        unsigned long int temp4 = !memcmp(&fs->win[BS_FilSysType32], "FAT32", 5);
        escreve_inteiro_lcd(temp4);
        posicao_cursor_lcd(2,0);
        escreve_frase_ram_lcd("BPB >          ");
        posicao_cursor_lcd(2,8);
        temp4 = !(fs->win[BPB_ExtFlags] & 0x80);
        escreve_inteiro_lcd(temp4);
        __delay_ms(3000);
        posicao_cursor_lcd(2,0);
        escreve_frase_ram_lcd("IF    >        ");
        posicao_cursor_lcd(2,8);
        temp4 = !memcmp(&fs->win[BS_FilSysType32], "FAT32", 5) && !(fs->win[BPB_ExtFlags] & 0x80);
        escreve_inteiro_lcd(temp4);
        __delay_ms(3000);
...
//		stat = disk_status(0); // está sendo chamado -- desabilitado para teste
		stat = 0;
...
FRESULT f_opendir (
	DIR_ *dj,			/* Pointer to directory object to create */
...
FRESULT f_readdir (
	DIR_ *dj,			/* Pointer to the directory object */
