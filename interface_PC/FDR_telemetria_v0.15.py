...
class Painel(wx.Frame):
    ...
    def InitUI(self):
        ...
        cont, titulo_bt, bt_caminho, diretorio = pn_se.arquivos("") # pegando os nomes e caminhos dos botoes
        ...
        x = 0
        if(cont == 0):
            self.statxt_01 = wx.StaticText(pn_se, -1, "A pasta \n" + str(diretorio) + "\nnão contém \nnenhum arquivo de \ntelemetria" , (16, 100), (-1, -1))
        else:
        ...
        for x in range(cont):
            count = count + 22
            nome_botao.append(str(pn_se.convert_data((titulo_bt[x]))))
            btn = wx.Button(pn_se, label=pn_se.convert_data((titulo_bt[x])), pos= (10, count), size = (-1,20))
            btn.Bind(wx.EVT_BUTTON, lambda evt, temp=pn_se.convert_data((titulo_bt[x])): OnButton(evt, temp) )
            ...
            def OnButton(Event, button_label):
                for y in range(len(nome_botao)):
                    if(button_label == nome_botao[y]):
                        # print(( y))
                        pn_se.abrir_recentes(y)
                        pn_sm.show_panel_one(self)
                        self.InitUI()
            ...
            if (x > 3 or x < 1):
                ...
                self.Bind(wx.EVT_BUTTON, pn_se.abrir_arquivos, botao_mais)
                pn_sm.show_panel_one(self)

                break
        if (x < 1):
            botao_mais = wx.Button(pn_se, wx.NewId(), "Abrir SD Card", (10, count + 32),(-1,20))
            self.Bind(wx.EVT_BUTTON, pn_se.abrir_diretorio, botao_mais)
            pn_sm.show_panel_one(self)
