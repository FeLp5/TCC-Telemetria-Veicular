...
class Painel_arquivos(wx.Panel):
    ...
    def mostrar_botoes(self, cont, titulo_bt, bt_caminho, diretorio):
        ...
        if(flag_iniciar == 0 and criou_botoes == 0):
            titulo_bt = [0] * 5
...
class Painel(wx.Frame):
    ...
    def InitUI(self):
        ...
        botao_infos = wx.Button(pn_ie, wx.NewId(), "Informações", (10, 20 ), (160,24))
        ...
        botao_mapa = wx.Button(pn_ie, wx.NewId(), "Trajeto", (10, 98 ), (160,24))
        botao_relatorio = wx.Button(pn_ie, wx.NewId(), "Relatório", (10, 124), (160,24))
        botao_grafico.Bind(wx.EVT_BUTTON, pn_sm.show_panel_two)
        botao_graficoRPM.Bind(wx.EVT_BUTTON, pn_sm.show_panel_tres)
        
        
        print "Flag iniciar " , flag_iniciar
        ...
        visualizar = wx.Menu()
        informacoes = visualizar.Append(wx.ID_ANY, "&Informações")
        ...
        # config = wx.Menu()
        # tema_es = config.Append(wx.ID_ANY, "&Tema Escuro")
        # tema_cl = config.Append(wx.ID_ANY, "&Tema Claro")
        # tema_sis = config.Append(wx.ID_ANY, "&Tema do Sistema")
        
        # Creating the menubar.
        menuBar = wx.MenuBar()
        menuBar.Append(filemenu,"&Arquivo") # Adding the "Arquivo" to the MenuBar
        menuBar.Append(visualizar,"&Visualizar") # Adding the "Visualizar" to the MenuBar
        # menuBar.Append(config,"&Configurações") # Adding the "Visualizar" to the MenuBar //não implementado ainda
        menuBar.Append(ajuda_menu,"&Ajuda") # Adding the "Visualizar" to the MenuBar //não implementado ainda


        self.SetMenuBar(menuBar)  # Adding the MenuBar to the Frame content.
        
        def status_botoes(self):
            print "FLAG INICIAR" , flag_iniciar, "CRIOU BOTOES", criou_botoes
            ...
            if flag_iniciar == 1:
                ...
                botao_infos.Bind(wx.EVT_BUTTON, pn_sm.show_panel_one)
            if arquivo_aberto == 0:
                botao_infos.Disable()
        ...
        hbox1.SetItemMinSize(pn_se, (180, int(t_y/3)))
        ...
        self.Bind(wx.EVT_MENU, pn_sm.show_panel_one, informacoes)
        self.Bind(wx.EVT_MENU, pn_sm.show_panel_two, g_velocidades)
        self.Bind(wx.EVT_MENU, pn_sm.show_panel_tres, g_rotacoes)
        self.Bind(wx.EVT_MENU, tema_escuro, g_mapa)
        self.Bind(wx.EVT_MENU, tema_escuro, relatorio)
