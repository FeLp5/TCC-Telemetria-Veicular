...
class Painel_arquivos(wx.Panel):
    ...
    def mostrar_botoes(self, cont, titulo_bt, bt_caminho, diretorio):
        ...
        titulo_botao_recebido = titulo_bt
        titulo_bt = [0] * 5
        ...
        if(titulo_botao_recebido):
            titulo_bt = titulo_botao_recebido
        
        global flag_iniciar
        
        if flag_iniciar == 1:
            self.botao0.SetLabel(str(self.convert_data((titulo_bt[0]))))
            self.botao1.SetLabel(str(self.convert_data((titulo_bt[1]))))
        ...
        if(cont == 0):
            # self.mostrar_painel("1")

            self.statxt_01 = wx.StaticText(self.panel_s1, -1, "A pasta \n" + str(diretorio) + "\nnão contém \nnenhum arquivo de \ntelemetria" , (16, 100), (-1, -1)) 
            self.botao_sdcard = wx.Button(self.panel_s1, wx.NewId(), "Abrir SD Card", (10, count + 32),(-1,20))
            ...
            self.statxt_02 = wx.StaticText(self.panel_s2, -1, "Trajetos" , (16, 10), (-1, -1))

            nome_botao = []
            
            #  desenha o botao e quebra o laco
            if (cont > 5):
                botao_mais = wx.Button(self.panel_s2, wx.NewId(), "Mais antigos", (10, 162),(-1,20))
            ...
            # self.show_panel_one(self)
                
    def mostrar_painel(self, tela):
