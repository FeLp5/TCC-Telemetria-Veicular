...
void inicializa_SPI(unsigned char SPI_mode)
{
    ...
    SSPCON1bits.SSPM0 = 0;              //Configura para usar o Timer 2
    SSPCON1bits.SSPM1 = 0;
    SSPCON1bits.SSPM2 = 0;
    SSPCON1bits.SSPM3 = 0;
...
char WriteSPI_(unsigned char dado)
{
    ...
    else
    {
        ...
        while(!PIR1bits.SSPIF && PIE1bits.SPPIE  );
//            posicao_cursor_lcd(1,0);
//            escreve_frase_ram_lcd("u are a genius");
        // wait until bus cycle complete  
    posicao_cursor_lcd(1,0);
    escreve_frase_ram_lcd("all workin"); 
