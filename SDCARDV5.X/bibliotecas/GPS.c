...
float get_sv(unsigned char date_pointer)
{
	...
	/* parse Longitude in GGA string stored in buffer */
	for( ; GGA_Buffer[index]!=','; index++){
		date_buffer[date_index]= GGA_Buffer[index];
		date_index++;
	}
    _date = atof(date_buffer);
	return _date;                 /* return integer raw value of Time */  
...
void tratamento_uart(unsigned char received_char)   
{
		...
		else if(RCM_CODE[0] == 'R' && RCM_CODE[1] == 'M' && RCM_CODE[2] == 'C'){ /* check for GGA string */
			IsItRCMString = 1;
			RCM_CODE[0] = 0; 
			RCM_CODE[1] = 0; 
			RCM_CODE[2] = 0;	
