...
static
FRESULT dir_read (
	DIR_E *dj,		/* Pointer to the directory object to store read object name */
	BYTE *dir		/* 32-byte working buffer */
)
{
	FRESULT res;
...
)
{
	...
	if (!_FS_32ONLY && !disk_readp(buf, sect, BS_FilSysType/*54*/, 2)  && LD_WORD(buf) == 0x4146)	/* Check FAT12/16 */
		return 0;
	if (_FS_FAT32  && !disk_readp(buf, sect, BS_FilSysType32/*82*/, 2)  && LD_WORD(buf) == 0x4146)	/* Check FAT32*/
		return 0;
	return 1;
}
...
)
{
	...
	fs->flag = 0;
	dj.fn = sp;
	res = follow_path(&dj, dir, "check.txt"/*path*/);	/* Follow the file path */
...
)
{
	...
	*bw = 0;
	...
	while (btw)	{									/* Repeat until all data transferred */
		...
		if (disk_writep(p, fs->dsect/*wcnt*/)) ABORT(FR_DISK_ERR);	/* Send data to the sector. Send sector data instead of word count*/
